...
//	cout<<"\r                                                                      \n ERROR occurred in "<<blocktype<<" block "<<blockname<<".\n See below for details or consult LOG.txt                       ";
    ...
    if (commandname != "") {
	startline += " in command " + commandname;
    }
...
int branchclass::getrootfreqs(string& testtree2)
{
    ...
    for (int pp = testtreesize; pp > 0; pp--) {
	if (testtree2[pp] == ')') {
	    pospos = pp;
	    break;
	}
    ...
    if (rootmodel == "") {
	controlerrorprint2("[BRANCHES]", name, "", "You haven't named a model for the root of this branch class.\nThe branch class tree should be of the form: (...)#m;\nwhere #m represents the (previously defined) root model m.", "");
	return -1;
    } else   {
	int ans = checkthere(rootmodel, totalmodelnames);

	if (ans == -1) {
	    controlerrorprint2("[BRANCHES]", name, "", "The model " + rootmodel + " in this branches block has not been defined", "");
	    return -1;
	} else   {
...
int branchclass::testgeneticcode(vector<int> modelpositions)
{
	...
	if (mygeneticcode != lastgeneticcode) {
	    controlerrorprint2("[BRANCHES]", name, "", "All models in a branches block should use the same genetic code.\nmodel " + n1 + " has genetic code " + d1 + "\nmodel " + n2 + " has genetic code " + d2 + "\nIf you want to allow the genetic code to change then use the command\n[BRANCHES*] instead of [BRANCHES] to turn off this safeguard.", "");
	    {
		if (breakonerror) {
		    return -1;
		}
...
int branchclass::testgeneticcode2(vector<int> modelpositions)
{
	...
	for (int qwe = 0; qwe < myallowedlist.size(); qwe++) {
	    if (lastgeneticcode == myallowedlist.at(qwe)) {
		codeerror = false;
		break;
	    }
	}
	if (codeerror) {
	    controlerrorprint2("[BRANCHES]", name, "", "All models in a branches block should use genetic codes that share \nthe same stop codons.\nmodel " + n1 + " has genetic code " + d1 + "\nmodel " + n2 + " has genetic code " + d2 + "\nThese codes do not share all stop codons and so cannot be combined. ", "");
	    {
		if (breakonerror) {
		    return -1;
		}
...
int branchclass::getbranchmodels(vector<string>& modelnames, vector<string>& totalmodelnames, vector<int>& modelpositions) //vector<model> &branchmodels)
{
	...
	if (found == -1) {
	    controlerrorprint2("[BRANCHES]", name, "", "The model " + thisname + " in this branches block has not been defined", "");
	    return -1;
	} else   {
	    //			if(type==3)
	    //			{
		...
		if (numcats != x) {
		    ...
		    string nn1 = totalmodelnames.at(found);
		    string nn2 = totalmodelnames.at(rootmodelpos);
...
int branchclass::buildbranches(string& testtree, string& branchesname, vector<string>& modelnames)
{
    ...
    if (bracketcount != 0) {
	controlerrorprint2("[BRANCHES]", name, "", "Number of parentheses in tree for this branch class do not match", "");
	return -1;
    }
	...
	if (testtree[0] != '(') {
		...
		if (modelnamewrite) {
		    modelnamestring += c1;
		}                                                       // cout<<"X "<<modelnamestring<<endl;}
		if ((c1 == '#') && !modelnamewrite) {
		    modelnamewrite = true;
		}
	    }
	...
	for (int kd = 0; kd < remaining.size(); kd++) {
	    buildbranches(remaining.at(kd), branchesname, modelnames);
	}
    }
...
int siteclass::testgeneticcode()
{
	...
	if (geneticcode != lastgeneticcode) {
	    controlerrorprint2("[SITES]", name, "", "All models/branches in a sites block should use the same genetic code.\n" + modelorbranch1 + " " + n1 + " has genetic code " + d1 + "\n" + modelorbranch2 + " " + n2 + " has genetic code " + d2, "");
	    {
		if (breakonerror) {
		    return -1;
		}
...
int siteclass::sortoutnames(vector<string>& mbtrees, vector<string>& totalmodelnames, vector<string>& totalbranchnames)
{
	...
	if (ans == -1) {
	    ...
	    if (ans == -1) {
		controlerrorprint2("[SITES]", name, "", "You specified \"" + mbname + "\" in this sites class.\nNo model/branch class with this name has been defined yet.", "");
		{
		    if (breakonerror) {
			return -1;
		    }
		...
		for (int o1 = 0; o1 < modpos.size(); o1++) {
		    ...
		    if (minicheck == -1) {
			allmodelsused.push_back(y);
		    }
		}
	    }

	    for (int yf = 0; yf < tree.size(); yf++) {
		char c = tree[yf];
		if ((c == '(') || (c == ')') || (c == ',') || (c == ';')) {
		    stree += c;
		}
...
int siteclass::sortouttrees(vector<string>& mbtrees)
{
	...
	if (testtree != "") {
		...
		if (s1s > s2s) {
		    for (int fv = 0; fv < s1s - s2s; fv++) {
			s2 += " ";
		    }
		} else   {
		    for (int fv = 0; fv < s2s - s1s; fv++) {
			s1 += " ";
		    }
		}
...
int teststring(int lastresult, string com, vector<string>& allowed, string blocktype, string allowedstring, string blockname)
{
	...
	if (!allAinB(com, allowedstring)) {
	    ...
	    string ds;
	    if (lastresult != -999) {
		ds = allowed.at(lastresult);
	    }

//			cout<<blocktype<<" "<<com<<" teststring func 1"<<endl;
	    if ((blocktype == "[TREE]") && (ds == "[user]")) {
		myresult = lastresult;
	    } else   {
		ifstream gf1;
		gf1.open(com.c_str());
		if (gf1.good()) {
		    myresult = lastresult;                                 // i.e. if it is a filename and the file exists and can be opened.
		} else   {
		    controlerrorprint2(blocktype, blockname, ds, "Input value must be a positive decimal number or a command [..]\nIf this was meant to be a filename no such file exists.", com);
		}
	    }
	} else   {
	    myresult = lastresult;
...
int dealwithsettings(vector<string>& block)
{
    ...
    if (size == 0) {
	controlerrorprint2("[SETTINGS]", "", "", "The settings block seems to be empty.", "");
	{
	    if (breakonerror) {
		return -1;
	    }
    ...
    if (size % 2 != 0) {
	controlerrorprint2("[SETTINGS]", "", "", "The settings block must contain an even number of statements:\n[COMMAND1] value1\n[COMMAND2] value2\nand so on....", "");
	{
	    if (breakonerror) {
		return -1;
	    }
    ...
    string scommands[15] =
    {
	"[printindelstatistics]", "[ancestralprint]",          "[phylipextension]", "[nexusextension]",        "[fastaextension]", "[output]",        "[randomseed]", "[printrates]",
	...
	s1 = block.at(j - 1);
	s2 = block.at(j);
	if (!allAinB(s2, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890,.;()-_")) {
	    controlerrorprint2("[SETTINGS]", "", "", "Values after commands should only contain ,.;()-_ \nand alpha-numeric characters.", "");
	    {
		if (breakonerror) {
		    return -1;
		}
	...
	if ((s1[0] != '[') || (s1[s1.size() - 1] != ']')) {
	    controlerrorprint2("[SETTINGS]", "", "", "Was expecting a command in square brackets e.g. [COMMAND]\nbut found: " + s1, "");
	    {
		if (breakonerror) {
		    return -1;
		}
	...
	if (commanderror == -1) {
	    controlerrorprint2("[SETTINGS]", "", "", "The command " + s1 + " is not valid in a setting block.\nPlease consult manual or example control file.", "");
	    {
		if (breakonerror) {
		    return -1;
		}
		...
		controlerrorprint2("[SETTINGS]", "", scommands[commanderror], "Value must be TRUE or FALSE for this command.", "");
		{
		    if (breakonerror) {
			return -1;
		    }
		...
		controlerrorprint2("[SETTINGS]", "", scommands[commanderror], "Value must be SAME, NEW or FALSE for this command.", "");
		{
		    if (breakonerror) {
			return -1;
		    }
	    ...
	    phylipextension = s2;
	} else if (commanderror == 3)   {
	    nexusextension = s2;
	} else if (commanderror == 4)   {
	    fastaextension = s2;
	    ...
	    if (!allAinB(s2, "1234567890-")) {
		controlerrorprint2("[SETTINGS]", "", scommands[commanderror], "This value must be an integer.", "");
		{
		    if (breakonerror) {
			return -1;
		    }
	    ...
	    if (s2 == "TRUE") {
		printcodonsasDNA = false;
	    } else if (s2 == "FALSE")   {
		printcodonsasDNA = true;
	    } else   {
		controlerrorprint2("[SETTINGS]", "", scommands[commanderror], "Value must be TRUE or FALSE for this command.", "");
		{
		    if (breakonerror) {
	    ...
	    if (s2 == "TRUE") {
		fixtrueproportions = true;
	    } else if (s2 == "FALSE")   {
		fixtrueproportions = false;
	    } else   {
		controlerrorprint2("[SETTINGS]", "", scommands[commanderror], "Value must be TRUE or FALSE for this command.", "");
		{
		    if (breakonerror) {
...
int dealwithmodel(vector<string>& block)
{
    ...
    int            ngamcat     = 5;
    double         codonrates[3];
    codonrates[0] = codonrates[1] = codonrates[2] = 1;
    vector<double> basefreqs, rootbasefreqs, insertfreqs, params;

    string commandsarray[143] =
    {
	"[insertmodel]", "[deletemodel]", "[indelmodel]", "[insertrate]", "[deleterate]", "[indelrate]",
	"[geneticcode]", "[submodel]",    "[rates]",      "[Crates]",     "[statefreq]",  "[istatefreq]","[rootstatefreq]","[XXXrootseq]",
	"[testindeldistributions]"
    };                                                       // acceptable list of commands
...
//	if(hg[0]!='#' || hg[hg.size()-1]!='#') { controlerrorprint2("[MODEL]", "?", "", "First statement in a [MODEL] block must be a model name statement in the form #modelname#",""); {if(breakonerror) return -1;} }
	...
	if (checkthere(mymodelname, totalmodelnames) != -1) {
	    controlerrorprint2("[MODEL]", mymodelname, "", "A model with this name has been specified twice.", "");
	    {
		if (breakonerror) {
		    return -1;
		}
	...
	if (hg[0] == '#') {
	    ...
	    if (mymodelname == name) {
		controlerrorprint2("[MODEL]", mymodelname, "", "You cannot copy a model within it's own definition statement!\nThe second model name should be different from " + mymodelname, "");
		{
		    if (breakonerror) {
			return -1;
		    }
	    ...
	    if (ans == -1) {
		controlerrorprint2("[MODEL]", name, "", "Cannot copy model " + mymodelname + ".\nNo model with this name has been defined yet.", "");
		{
		    if (breakonerror) {
			return -1;
		    }
	...
	for (int i1 = astart; i1 < blocksize + 1; i1++) {
	    ...
	    if ((oldhg == "[insertmodel]") || (oldhg == "[deletemodel]") || (oldhg == "[indelmodel]")) {
		myallowed = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890.";                                                                           //"NBUSEROLDPOWCUMDAWGLAV";
	    }
		...
		if ((mytest == lasttest) && (i1 != blocksize)) {
		    tempvec.push_back(hg);
		}                        // cout<<"H1 "<<hg<<endl;}
		else if (i1 != astart) { //a2
		    if (tempvec.size() == 0) {
			...
			// Negative Binomial Distribution
			    ...
			    if (myin == "NB") {
				myindelmodelint = 1;                    // used in program
			    } else   {
				myindelmodelint = 11;                   // alternative parameterisation for testing
			    }
			    if (tempsize != 3) {
				noindelerror = false;
			    }
			...
			// Zipfian Distribution (Power Law)
			    ...
			    if (myin == "POW") {
				myindelmodelint = 2;                // Fast Zipf
			    }
			    if (myin == "POWDAWG") {
				myindelmodelint = 13;               // Slower Zipf from DAWG - here for checking fast zipf!
			    }
			    if (myin == "POWCUM") {
				myindelmodelint = 12;               // cumulative method as for USER and LAV used to check both above!
			    }
			...
			if (lasttest == 6) {
			    ...
			    if (AinB('.', myin)) {
				controlerrorprint2("[MODEL]", name, commands.at(lasttest), "Value for this command should not be decimal, was expecting integer.", myin);
				{
				    if (breakonerror) {
					return -1;
				    }
				...
				if (((myout > 0) && (myout < 7)) || ((myout > 8) && (myout < 17)) || ((myout > 20) && (myout < 24))) {
				    geneticcode = myout;
				} else   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "Value for this command should be an integer 1-6, 9-16, or 21-23\nThese correspond to the entries on GenBank (October 2008).", myin);
				    {
					if (breakonerror) {
					    return -1;
					}
			    ...
			    if (myout < 0) {
				controlerrorprint2("[MODEL]", name, commands.at(lasttest), "Value for this command should be greater than or equal to zero.", myin);
				{
				    if (breakonerror) {
					return -1;
				    }
			...
			if (!allAinB(myin, "0123456789.")) {
				...
				while (ig1.good()) {
					...
					if (writeon) {
					    ...
					    aamodel.push_back(atof(sss.c_str()));
					    sss = "";
					}
				    } else   {
					controlerrorprint2("[MODEL]", name, "[submodel]", "Reading user defined protein substitution model from file:\n" + myin + "\n...but have found illegal character \"" + c2 + "\"\nEntries in this file can only be decimal numbers separated by\nspaces, horizontal tabs and new lines.", "");
					{
					    if (breakonerror) {
						return -1;
					    }
				...
				if ((mymodel > -1) && (mymodel < b) && !AinB('.', myin)) {
				    ...
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "First value for this command represents the model number.\nThis should be a integer between 0 and " + bnum + " inclusive\nwhen type is set to " + typestring + ".", myin);
				    {
					if (breakonerror) {
					    return -1;
					}
				...
				if ((mymodel == 0) && (nstsize != 1)) {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "NUCLEOTIDE substitution model set as JC69. \nNot expecting any substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 1) && (nstsize != 1))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "NUCLEOTIDE substitution model set as F81. \nNot expecting any substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 2) && (nstsize != 2) && (nstsize != 3))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "NUCLEOTIDE substitution model set as K80. \nExpecting 1 or 2 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 3) && (nstsize != 2) && (nstsize != 3))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "NUCLEOTIDE substitution model set as HKY85. \nExpecting 1 or 2 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 4) && (nstsize != 3) && (nstsize != 4))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "NUCLEOTIDE substitution model set as TN93ef. \nExpecting 2 or 3 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 5) && (nstsize != 3) && (nstsize != 4))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "NUCLEOTIDE substitution model set as TN93. \nExpecting 2 or 3 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 6) && (nstsize != 3) && (nstsize != 4))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "NUCLEOTIDE substitution model set as K81. \nExpecting 2 or 3 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 7) && (nstsize != 3) && (nstsize != 4))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "NUCLEOTIDE substitution model set as K81uf. \nExpecting 2 or 3 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 8) && (nstsize != 4) && (nstsize != 5))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "NUCLEOTIDE substitution model set as TIMef. \nExpecting 3 or 4 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 9) && (nstsize != 4) && (nstsize != 5))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "NUCLEOTIDE substitution model set as TIM. \nExpecting 3 or 4 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 10) && (nstsize != 5) && (nstsize != 6))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "NUCLEOTIDE substitution model set as TVMef. \nExpecting 4 or 5 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 11) && (nstsize != 5) && (nstsize != 6))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "NUCLEOTIDE substitution model set as TVM. \nExpecting 4 or 5 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 12) && (nstsize != 6) && (nstsize != 7))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "NUCLEOTIDE substitution model set as SYM. \nExpecting 5 or 6 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 13) && (nstsize != 6) && (nstsize != 7))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "NUCLEOTIDE substitution model set as GTR. \nExpecting 5 or 6 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 14) && (nstsize != 2) && (nstsize != 3))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "NUCLEOTIDE substitution model set as F84ef. \nExpecting 1 or 2 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 15) && (nstsize != 2) && (nstsize != 3))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "NUCLEOTIDE substitution model set as F84. \nExpecting 1 or 2 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				...
				if ((mymodel == 0) && (nstsize != 2)) {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "CODON substitution model set as M0. \nWas expecting 2 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 1) && (nstsize != 3))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "CODON substitution model set as M1. \nWas expecting 3 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 2) && (nstsize != 5))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "CODON substitution model set as M2. \nWas expecting 5 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
					...
					if (AinB('.', tempvec.back())) {
					    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "CODON substitution model set as M5.\nWhen specifying 4 substitution parameters after the model number the\nlast value represents the number of categories to use in the discrete \napproximation for any continuous distributions.  This must be an integer. ", tempvec.back());
					    {
						if (breakonerror) {
						    return -1;
						}
					    ...
					    ngamcat = atoi((tempvec.back()).c_str());
					    params.pop_back();
					}
				    } else   {
					if (nstsize != 3) {
				    ...
				    if (nstsize == 6) {
					if (AinB('.', tempvec.back())) {
					    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "CODON substitution model set as M6.\nWhen specifying 6 substitution parameters after the model number the\nlast value represents the number of categories to use in the discrete \napproximation for any continuous distributions.  This must be an integer. ", tempvec.back());
					    {
						if (breakonerror) {
						    return -1;
						}
				    ...
				    if (nstsize == 4) {
					if (AinB('.', tempvec.back())) {
					    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "CODON substitution model set as M7.\nWhen specifying 4 substitution parameters after the model number the\nlast value represents the number of categories to use in the discrete \napproximation for any continuous distributions.  This must be an integer. ", tempvec.back());
					    {
						if (breakonerror) {
						    return -1;
						}
				    ...
				    if (nstsize == 6) {
					if (AinB('.', tempvec.back())) {
					    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "CODON substitution model set as M8.\nWhen specifying 6 substitution parameters after the model number the\nlast value represents the number of categories to use in the discrete \napproximation for any continuous distributions.  This must be an integer. ", tempvec.back());
					    {
						if (breakonerror) {
						    return -1;
						}
				    ...
				    if (nstsize == 7) {
					if (AinB('.', tempvec.back())) {
					    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "CODON substitution model set as M9.\nWhen specifying 7 substitution parameters after the model number the\nlast value represents the number of categories to use in the discrete \napproximation for any continuous distributions.  This must be an integer. ", tempvec.back());
					    {
						if (breakonerror) {
						    return -1;
						}
				    ...
				    if (nstsize == 7) {
					if (AinB('.', tempvec.back())) {
					    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "CODON substitution model set as M10. \nWhen specifying 7 substitution parameters after the model number the\nlast value represents the number of categories to use in the discrete \napproximation for any continuous distributions.  This must be an integer. ", tempvec.back());
					    {
						if (breakonerror) {
						    return -1;
						}
				    ...
				    if (nstsize == 7) {
					if (AinB('.', tempvec.back())) {
					    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "CODON substitution model set as M11. \nWhen specifying 7 substitution parameters after the model number the\nlast value represents the number of categories to use in the discrete \napproximation for any continuous distributions.  This must be an integer. ", tempvec.back());
					    {
						if (breakonerror) {
						    return -1;
						}
				    ...
				    if (nstsize == 7) {
					if (AinB('.', tempvec.back())) {
					    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "CODON substitution model set as M12. \nWhen specifying 7 substitution parameters after the model number the\nlast value represents the number of categories to use in the discrete \napproximation for any continuous distributions.  This must be an integer. ", tempvec.back());
					    {
						if (breakonerror) {
						    return -1;
						}
				    ...
				    if (nstsize == 8) {
					if (AinB('.', tempvec.back())) {
					    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "CODON substitution model set as M13. \nWhen specifying 8 substitution parameters after the model number the\nlast value represents the number of categories to use in the discrete \napproximation for any continuous distributions.  This must be an integer. ", tempvec.back());
					    {
						if (breakonerror) {
						    return -1;
						}
				...
				else if ((mymodel == 4) && (nstsize < 3)) {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "CODON substitution model set as M4. \nWas expecting at least 2 substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 14) && (nstsize != 0))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "CODON substitution model set as ECMrest. \nNot expecting any substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				    }
				} else if ((mymodel == 15) && (nstsize != 0))   {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "CODON substitution model set as ECMunrest. \nNot expecting any substitution parameters.\nInstead found " + fh + " parameters after model number", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				...
				if ((mymodel == 12) || (mymodel == 13) || (mymodel == 2)) {
				    if (params.at(1) > 1) {
					controlerrorprint2("[MODEL]", name, commands.at(lasttest), "proportion p0 must be between 0 and 1.  You entered " + tempvec.at(2), "");
					{
					    if (breakonerror) {
						return -1;
					    }
				    ...
				    if (params.at(2) > 1) {
					controlerrorprint2("[MODEL]", name, commands.at(lasttest), "proportion p1 must be between 0 and 1.  You entered " + tempvec.at(3), "");
					{
					    if (breakonerror) {
						return -1;
					    }
				    ...
				    if ((mymodel != 12) && (params.at(1) + params.at(2) > 1)) {
					controlerrorprint2("[MODEL]", name, commands.at(lasttest), "sum of proportions p0 and p1 must be between 0 and 1.  \nYou entered " + tempvec.at(2) + " and " + tempvec.at(3), "");
					{
					    if (breakonerror) {
						return -1;
					    }
					...
					controlerrorprint2("[MODEL]", name, commands.at(lasttest), "sum of proportions must be between 0 and 1.  It was " + hf + ".", "");
					{
					    if (breakonerror) {
						return -1;
					    }
...
///////////////////////////////////////////////
			...
			if (model_type == model::Type::codon) {
			    ...
			    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "There should be 3 values following a [rates] command: pinv alpha ngamcat.", "");
			    {
				if (breakonerror) {
				    return -1;
				}
			    ...
			    if (AinB('.', tempvec.at(2))) {
				controlerrorprint2("[MODEL]", name, commands.at(lasttest), "The 3rd value representing ngamcat should be an integer", tempvec.at(2));
				{
				    if (breakonerror) {
					return -1;
				    }
			    ...
			    if (pinv > 1) {
				controlerrorprint2("[MODEL]", name, commands.at(lasttest), "The 1st value representing pinv should be between 0 and 1.", tempvec.at(0));
				{
				    if (breakonerror) {
					return -1;
				    }
			...
			if (tempvec.size() != 3) {
			    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "There should be 3 decimal values following a [Crates] command.", "");
			    {
				if (breakonerror) {
				    return -1;
				}
			...
			if (errorprint) {
			    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "There should be " + mysizes + " values for a base frequency command\nwhen simulating " + mytype + " data. You entered " + mysizeS + " values.", "");
			    {
				if (breakonerror) {
				    return -1;
				}
				...
				if (mytemp < 0) {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "Values for base frequency commands must be positive.", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				...
				for (int gh1q = 0; gh1q < mysize; gh1q++) {
				    scaler += tempbases.at(gh1q);
				}
				for (int gh2q = 0; gh2q < mysize; gh2q++) {
				    tempbases.at(gh2q) /= scaler;
				}
				...
				if (diff > 0.0001) {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "F1X4 base frequencies did not add up to 1 so they have been rescaled.", "");
				}
				...
				for (int gh1a = 0; gh1a < 4; gh1a++) {
				    scaler += tempbases.at(gh1a);
				}
				for (int gh2a = 0; gh2a < 4; gh2a++) {
				    tempbases.at(gh2a) /= scaler;
				}
				...
				if (diff > 0.0001) {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "F3X4 base frequencies for the 1st codon position\ndid not add up to 1 so they have been rescaled.", "");
				}

				scaler = 0;
				for (int gh1b = 4; gh1b < 8; gh1b++) {
				    scaler += tempbases.at(gh1b);
				}
				for (int gh2b = 4; gh2b < 8; gh2b++) {
				    tempbases.at(gh2b) /= scaler;
				}
				...
				if (diff > 0.0001) {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "F3X4 base frequencies for the 2nd codon position\ndid not add up to 1 so they have been rescaled.", "");
				}

				scaler = 0;
				for (int gh1c = 8; gh1c < 12; gh1c++) {
				    scaler += tempbases.at(gh1c);
				}
				for (int gh2c = 8; gh2c < 12; gh2c++) {
				    tempbases.at(gh2c) /= scaler;
				}
				...
				if (diff > 0.0001) {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "F3X4 base frequencies for the 3rd codon position\ndid not add up to 1 so they have been rescaled.", "");
				}
				...
				for (int gh1s = 0; gh1s < mysize; gh1s++) {
				    scaler += tempbases.at(gh1s);
				}
				for (int gh2s = 0; gh2s < mysize; gh2s++) {
				    tempbases.at(gh2s) /= scaler;
				}
				...
				if (diff > 0.0001) {
				    controlerrorprint2("[MODEL]", name, commands.at(lasttest), "Base frequencies did not add up to 1 so they have been rescaled.", "");
				}
				...
				for (int gh2 = 0; gh2 < mysize; gh2++) {
				    tempbases2.at(gh2) /= scaler;
				}
				...
				insertfreqs = tempbases2;
			    } else if (lasttest == 12)   {
				rootbasefreqs = tempbases2;
			    }
			} //x2
		    }     //x1
...
int dealwithsites(vector<string>& block)
{
    ...
    if (mymy == 0) {
	controlerrorprint2("[SITES]", "", "", "You must specify a name for a sites block. This one is empty.", "");
	{
	    if (breakonerror) {
		return -1;
	    }
	...
	name = block.at(0);
	if (!allAinB(name, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890,.;()-_")) {
	    controlerrorprint2("[SITES]", name, "", "First statement in a [SITES] block must be a sites name statement.\nThe name should only contain  ,.;()-_ and alpha-numeric characters.", "");
	    {
		if (breakonerror) {
		    return -1;
		}
	...
	if (checkthere(name, totalsitenames) != -1) {
	    controlerrorprint2("[SITES]", name, "", "A sites class with this name has been specified twice.", "");
	    {
		if (breakonerror) {
		    return -1;
		}
    ...
    if (mymy == 1) {
	controlerrorprint2("[SITES]", name, "", "This sites block contains no commands.", "");
	{
	    if (breakonerror) {
		return -1;
	    }
	...
	for (int y1 = 0; y1 < a.size(); y1++) {
	    ...
	    if ((c == ',') || (c == ']')) {
		if (allAinB(b, "0.123456789")) {
		    controlerrorprint2("[SITES]", name, "", "This model/branchclass name is a decimal number, is this a mistake?\n\"" + a + "\" cannot be used as a model/branchclass name.", "");
		    {
			if (breakonerror) {
			    return -1;
			}
		...
		if (!allAinB(b, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890,.;()-_")) {
		    controlerrorprint2("[SITES]", name, "", "Model/Branchclass names should only contain ,.;()-_ and alpha-numeric characters.", "");
		    {
			if (breakonerror) {
			    return -1;
			}
	...
	controlerrorprint2("[SITES]", name, "", "Sites blocks must have an even number of statements in pairs of the form:\n'proportion model' or 'proportion branchclass' e.g. 0.2 mymodelname.", "");
	{
	    if (breakonerror) {
		return -1;
	    }
	...
	controlerrorprint2("[SITES]", name, "", "Sites blocks need at least two site classes otherwise\njust evolve a model or a branchclass not a siteclass!", "");
	{
	    if (breakonerror) {
		return -1;
	    }
	...
	for (int i1 = 1; i1 < mymy; i1++) {
	    ...
	    if ((a[0] == '[') && (a[a.size() - 1] == ']')) {
		controlerrorprint2("[SITES]", name, a, "Sites blocks do not contain other commands.\nHave you made a spelling mistake?", "");
		{
		    if (breakonerror) {
			return -1;
		    }
		...
		if (!allAinB(a, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890,.;()-_")) {
		    controlerrorprint2("[SITES]", name, "", "Model/Branchclass names should only contain ,.;()-_ and alpha-numeric characters.", "");
		    {
			if (breakonerror) {
			    return -1;
		...
		if (!allAinB(a, "0123456789.")) {
		    controlerrorprint2("[SITES]", name, "", "Possible Spelling Mistake...\nProportions should be decimal numbers, control file had:\n" + a + "\nIs this a model name or branch class name?\nIf so, the proportion should come before the name.", "");
		    {
			if (breakonerror) {
			    return -1;
			}
	...
	if (diff > 0.0001) {
		...
		props.at(j) /= sum;
	    }                                                                // cout<<props.at(j)<<" ";} cout<<endl;
	}

	for (int k = 1; k < props.size(); k++) {
	    props.at(k) += props.at(k - 1);
	}
	...
	if (diff > 0.0001) {
	    cout << "INTERNAL ERROR in rescaling site class proportions." << endl;
	}
...
int dealwithbranches(vector<string>& block, bool iscodefixed)
{
    ...
    if (mymy == 0) {
	controlerrorprint2("[BRANCHES]", "", "", "You must specify a name for a branches block. This one is empty.", "");
	{
	    if (breakonerror) {
		return -1;
	    }
	...
	name = block.at(0);
	if (!allAinB(name, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890,.;()-_")) {
	    controlerrorprint2("[BRANCHES]", name, "", "First statement in a [BRANCHES] block must be a branches name statement.\nThe name should only contain  ,.;()-_ and alpha-numeric characters.", "");
	    {
		if (breakonerror) {
		    return -1;
		}
	...
	if (checkthere(name, totalbranchesnames) != -1) {
	    controlerrorprint2("[BRANCHES]", name, "", "A branches class with this name has been specified twice.", "");
	    {
		if (breakonerror) {
		    return -1;
		}
    ...
    if (mymy == 1) {
	controlerrorprint2("[BRANCHES]", name, "", "This branches block contains no tree.", "");
	{
	    if (breakonerror) {
		return -1;
	    }
	...
	string s = block.at(i);
	if ((s[0] == '[') && (s[s.size() - 1] == ']')) {
	    controlerrorprint2("[BRANCHES]", name, s, "Branch blocks do not contain other commands.\nHave you made a spelling mistake?", "");
	    {
		if (breakonerror) {
		    return -1;
		}
...
//		cout<<"branches "<<block.at(i)<<endl;
	...
	controlerrorprint2("[BRANCHES]", name, "", "User defined tree does not appear to be a NEWICK format tree (...); ", "");
	{
	    if (breakonerror) {
		return -1;
	    }
	...
	treeerror = true;
	controlerrorprint2("[BRANCHES]", name, "", "Number of parantheses in guide tree do not match.", "");
	{
	    if (breakonerror) {
		return -1;
	    }
    ...
    if (mybracketleft == 0) {
	treeerror = true;
	controlerrorprint2("[BRANCHES]", name, "", "Branch class guide tree must contain as least one set of parentheses.", "");
	{
	    if (breakonerror) {
		return -1;
	    }
	...
	if ((c == ',') || (c == ')')) {
	    if (hashcount == 0) {
		controlerrorprint2("[BRANCHES]", name, "", "Branch class guide tree contains a branch without a model name.\nModel names must have a # character in front of them.", "");
		{
		    if (breakonerror) {
			return -1;
		    }
	    ...
	    if (hashcount > 1) {
		controlerrorprint2("[BRANCHES]", name, "", "Branch class guide tree contains a branch with more than one model name.\nModel names must be preceded with one single #.", "");
		{
		    if (breakonerror) {
			return -1;
		    }
...
//			if(seedgiven) {mymtrand.seed(seed);tester.seed(seed);}// cout<<"name "<<name<<" SEEDDDD "<<seed<<endl;}
    ...
    if (!treerandom) {
	if (doctorlengths == "ULTRAMETRIC") {
	    tree = randomise_ultrametric(tree);
	}
...
//			cout<<"R"<<max_distance<<"R"<<endl<<endl;
	...
	if (scalerm != -1) {
	    tree = rescaledtree(tree, max_distance, scalerm);
	}
	if (scaler != -1) {
	    tree = rescaledtree(tree, treelength, scaler);
	}
	if (scalerd != -1) {
	    tree = rescaledtree(tree, treedepth, scalerd);             ////tree=redepthtree(tree, gettreelength(tree), scaler);
	}
	max_distance = 0;
	treedepth    = getmaxtreedepth(tree);
...
//			cout<<"T"<<max_distance<<"T"<<endl<<endl;
	...
	if (scaler != -1) {
	    double diff = treelength - scaler;
	    if (diff < 0) {
		diff = -diff;
	    }
	    if (diff > 0.0001) {
		controlerrorprint2("[TREE]", name, "", "The treelength adjustment is not within the limits I expected.\n Please report this and it will be promptly fixed!", "");
	    }
	}
	if (scalerd != -1) {
	    double diff = treedepth - scalerd;
	    if (diff < 0) {
		diff = -diff;
	    }
	    if (diff > 0.0001) {
		controlerrorprint2("[TREE]", name, "", "The treedepth adjustment is not within the limits I expected.\n Please report this and it will be promptly fixed!", "");
	    }
	}
	if (scalerm != -1) {
	    double diff = max_distance - scalerm;
	    if (diff < 0) {
		diff = -diff;
	    }
	    if (diff > 0.0001) {
		controlerrorprint2("[TREE]", name, "", "The maxdistance adjustment is not within the limits I expected.\n Please report this and it will be promptly fixed!", "");
	    }
	}
    }
}
...
void Tree::newrandom(int rep) //, unsigned long &last)
{
    ...
    int theseed;
    if (seedgiven) {
	theseed = mymtrand.randInt();
    } else   {
	theseed = mtrand1.randInt();
    }
...
//		cout<<"Q "<<temptree<<endl;
    ...
    tree = "";
    for (int yq = 0; yq < temptree.size(); yq++) {
	if (temptree[yq] != 'S') {
	    tree += temptree[yq];
	}
    }
...
//		cout<<"W "<<tree<<endl;
    ...
    if (scaler != -1) {
	tree = rescaledtree(tree, gettreelength(tree), scaler);
    }

    if (scalerd != -1) {
	tree = rescaledtree(tree, getmaxtreedepth(tree), scalerd);
    }
...
double Tree::correctlastdepth(double maxdepth, string originaltree, double depthsofar, vector<string>& originals, vector<string>& changes)
{
	...
	for (pi = 1; pi < originaltree.size(); pi++) {
	    ...
	    if ((bracketcount == 1) && (c == ',')) {
		bits.push_back(s); /*cout<<"S"<<s<<"S"<<endl;*/ s = "";
		continue;
	    }
	    if (bracketcount == 0) {
		bits.push_back(s); /* cout<<"SS"<<s<<"SS"<<endl; */ break;
	    }
...
//			{
	...
	pi++;
	if (originaltree[pi] != ':') {
	    controlerrorprint2("[TREE]", name, "", "Something is wrong in the getnextdepthcommand.\n Please report this and it will be promptly fixed!", "");
	}


	pi++;
	for (int yg = pi; yg < originaltree.size(); yg++) {
	    s += originaltree[yg];
	}
...
//			}
	...
	for (pi = 0; pi < bits.size(); pi++) {
	    correctlastdepth(maxdepth, bits.at(pi), depthsofar, originals, changes);
	}
    } else   {
	//end of the tree!!!
...
string Tree::randomise_ultrametric(string originaltree)
{
	...
	for (int fr = 0; fr < sizey; fr++) {
	    tree += originaltree[fr];
	}
	tree += ":0.0";
    } else   {
	tree = originaltree;
	...
	if ((c == ',') || (c == ')')) {
		...
		if (mylength == onestochange.at(rf)) {
		    pos = rf; /*cout<<whattochangeto.at(pos)<<endl;*/ break;
		}
		//cout<<endl;
	    }
	    if (pos == -1) {
...
// these two lines ended up stuck in a loop on very deep trees!
	...
	for (pi = 0; pi < sizey; pi++) {
	    ...
	    if (mydepths.at(pi) > mymax) {
		mymax = mydepths.at(pi);
	    }
...
double Tree::gettreelength(string originaltree)
{
	...
	if ((c == ',') || (c == ')')) {
	    branch = 0;
	    double length = atof(mylength.c_str());
	    treelength += length;
	    mylength    = "";
	}
	if (branch == 1) {
...
int dealwithtrees(vector<string>& block)
{
    ...
    if (mysize == 0) {
	controlerrorprint2("[TREE]", "", "", "No tree name has been given", "");
	{
	    if (breakonerror) {
		return -1;
	    }
    ...
    if (mysize == 1) {
	controlerrorprint2("[TREE]", name, "", "The tree block is empty.", "");
	{
	    if (breakonerror) {
		return -1;
	    }
    ...
    if (mysize == 2) {
	if (block.at(1) == "[user]") {
	    controlerrorprint2("[TREE]", name, "", "There is no tree in the tree block.", "");
	    {
		if (breakonerror) {
		    return -1;
		}
    ...
    if (lastbit[0] == '[') {
	controlerrorprint2("[TREE]", name, lastbit, "The last entry in a [TREE] block cannot be a command.", "");
	{
	    if (breakonerror) {
		return -1;
	    }
	...
	for (int p3 = 1; p3 < 4; p3++) {
	    if (well[p3]) {
		controlerrorprint2("[TREE]", name, commands.at(p3), "This command cannot be used in a user-defined tree block.", commands.at(p3));
		{
		    if (breakonerror) {
			return -1;
		    }
	...
	for (int i1 = 1; i1 < blocksize + 1; i1++) {
	    ...
	    if ((hg == "NON-ULTRAMETRIC") || (hg == "ULTRAMETRIC")) {
		doultra = true;
	    }
	}
    }
    ...
    if (!(well[0]) && well[6]) {
	controlerrorprint2("[TREE]", name, commands.at(6), "This command cannot be used in a random tree block.", commands.at(6));
	{
	    if (breakonerror) {
		return -1;
	    }
...
//	if(hg[0]!='#' || hg[hg.size()-1]!='#') { controlerrorprint2("[MODEL]", "?", "", "First statement in a [MODEL] block must be a model name statement in the form #modelname#",""); {if(breakonerror) return -1;} }
    if (!allAinB(hg, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890,.;()-_")) {
	controlerrorprint2("[TREE]", name, "", "First statement in a [TREE] block must be a tree name statement.\nThe name should only contain  ,.;()-_ and alpha-numeric characters.", hg);
	{
	    if (breakonerror) {
		return -1;
	    }
	...
	if (checkthere(name, totaltreenames) != -1) {
	    controlerrorprint2("[TREE]", name, "", "A tree with this name has been specified twice.", "");
	    {
		if (breakonerror) {
		    return -1;
		}
	...
	for (int i1 = 1; i1 < blocksize + 1; i1++) {
	    ...
	    lasttest = mytest;
	    if (block.at(i1 - 1) != "[branchlengths]") {
		mytest = teststring(lasttest, hg, commands, "[TREE]", "0123456789.", name);
	    }
		...
		if ((mytest == lasttest) && (i1 != blocksize)) {
		    ...
		    if (tempsize == 0) {
			controlerrorprint2("[TREE]", name, commands.at(lasttest), "No values were found after this command in the control file.", "");
			{
			    if (breakonerror) {
				return -1;
			    }
			...
			if (tempsize != 1) {                     /* cout<<tempvec.at(0)<<" "<<tempvec.at(1)<<" "<<tempvec.at(1 )<<" E"<<endl;*/
			    controlerrorprint2("[TREE]", name, commands.at(lasttest), "There should be only one value after this command in the control file.", "");
			    {
				if (breakonerror) {
				    return -1;
				}
			...
			if (lasttest == 3) {
			    if (AinB('.', bit)) {
				controlerrorprint2("[TREE]", name, commands.at(lasttest), "First value after this command should be an integer.", bit);
				{
				    if (breakonerror) {
					return -1;
				    }
			    ...
			    if ((bit == "ULTRAMETRIC") || (bit == "NON-ULTRAMETRIC") || (bit == "EQUAL")) {
				doctorlengths = bit;
			    } else   {
				controlerrorprint2("[TREE]", name, commands.at(lasttest), "This command can take the following values\n\"ULTRAMETRIC\"\n\"NON-ULTRAMETRIC\"\n\"EQUAL\"\nYou entered:", bit);
				{
				    if (breakonerror) {
					return -1;
				    }
			...
			if (lasttest == 1) {
			    guidetreeroot = 2;                                   //rooted
			} else   {
			    guidetreeroot = 1;                                   //unrooted
			}
			string bit = tempvec.at(0);
			...
			string rawvalue;
			if (tempvec.size() == 1) {
			    rawvalue = tempvec.at(0);
			}                                                                      // cout<<"RAWVALUE 1 "<<rawvalue<<endl;}
			else {
...
// observed to evolve along the tree for testing.
			...
			if ((rawvalue[0] != '(') || (rawvalue[size - 1] != ';') || (rawvalue[size - 2] != ')')) {
			    controlerrorprint2("[TREE]", name, commands.at(lasttest), "User defined tree does not appear to be a NEWICK format tree (...); ", rawvalue);
			    {
				if (breakonerror) {
				    return -1;
				}
			...
			if (mybracketleft != mybracketright) {
			    ...
			    treeerror = true;
			    controlerrorprint2("[TREE]", name, commands.at(lasttest), "Number of parantheses in guide tree do not match.", "");
			    {
				if (breakonerror) {
				    return -1;
				}
			...
			if (mybracketleft == 0) {
			    treeerror = true;
			    controlerrorprint2("[TREE]", name, commands.at(lasttest), "Guide tree must contain as least one set of parentheses.", "");
			    {
				if (breakonerror) {
				    return -1;
				}
			...
			for (int fg2 = 0; fg2 < size; fg2++) {
				...
				for (int yg = 0; yg < currentlength.size(); yg++) {
				    ...
				    if (!AinB(c, "0.123456789:")) {
					treeerror = true;
					controlerrorprint2("[TREE]", name, commands.at(lasttest), "A branch length in the guide tree appears non-numerical", "");
					{
					    if (breakonerror) {
						return -1;
					    }
				...
				if (c1 != ':') {
				    treeerror = true;
				    controlerrorprint2("[TREE]", name, commands.at(lasttest), "There is no branch length after a right paranthesis in the tree.\nIt happened here \"" + lasterror + "\"", "");
				    {
					if (breakonerror) {
					    return -1;
					}
			    ...
			    if ((c1 == ')') || (c1 == ',')) {
				if (!AinB(c2, "0123456789")) {
				    treeerror = true;
				    controlerrorprint2("[TREE]", name, commands.at(lasttest), "There is a missing branch length in the tree.\nIt happened here \"" + lasterror + "\"", "");
				    {
					if (breakonerror) {
					    return -1;
					}
				...
				treeerror = true;
				controlerrorprint2("[TREE]", name, commands.at(lasttest), "There is a missing branch length after a : in the guide tree", "");
				{
				    if (breakonerror) {
					return -1;
				    }
			...
			if (!treeerror) {
				...
				if ((extbranch == 2) || (extbranch == 3)) {  //cout<<"Guide tree of "<<extbranch<<" taxa read in successfully:"<<endl<<" "<<rawvalue<<endl<<" "<<endl<<" ";
					...
					//cout<<extbranch<<" taxa ";
					if (coloncount != 2 * extbranch - 3 + rootvalue) {
					    guidetreebinary = false;
					} else   {
					    guidetreebinary = true;
					}
					if (rootbranches == 1) {
					    guidetreerooted = true;
					}
					if (rootbranches == 2) {
					    guidetreerooted = false;
					}
    ...
    if ((scaler != -1) && (scalerd != -1)) {
	controlerrorprint2("[TREE]", name, "[treedepth]", "You cannot use [treelength] and [treedepth] at the same time!", "");
	{
	    if (breakonerror) {
		return -1;
	    }
    ...
    if ((scaler != -1) && (scalerm != -1)) {
	controlerrorprint2("[TREE]", name, "[treelength]", "You cannot use [treelength] and [maxdistance] at the same time!", "");
	{
	    if (breakonerror) {
		return -1;
	    }
    ...
    if ((scalerm != -1) && (scalerd != -1)) {
	controlerrorprint2("[TREE]", name, "[treedepth]", "You cannot use [maxdistance] and [treedepth] at the same time!", "");
	{
	    if (breakonerror) {
		return -1;
	    }
    ...
    }                                                                                            // random trees always ultrametric, so max. pairwise distance i
...
//		isitrandomvec=myisitrandomvec;
	...
	if (mbstype == "model") {
	    isitsites.push_back(false);
	    isitbranches.push_back(false);
	}
	if (mbstype == "branches") {
	    isitsites.push_back(false);
	    isitbranches.push_back(true);
	}
	if (mbstype == "sites") {
...
int partitionclass::checktaxaintrees(vector<int> posvec)
{
	...
	if (t.ntaxa != ntaxa) {
	    ...
	    n1 << ntaxa;
	    n2 << t.ntaxa;
	    string s1    = n1.str(), s2 = n2.str();
	    string name1 = (totaltrees.at(posvec.at(0))).name, name2 = (totaltrees.at(posvec.at(p))).name;
...
int parse_partition_block(vector<string>& block)
{
    ...
    if (mymy == 0) {
	controlerrorprint2("[PARTITIONS]", "", "", "You must specify a name for a partitions block. This one is empty.", "");
	{
	    if (breakonerror) {
		return -1;
	    }
	...
	name = block.at(0);
	if (!allAinB(name, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890,.;()-_")) {
	    controlerrorprint2("[SITES]", name, "", "First statement in a [PARTITIONS] block must be a name statement.\nThe name should only contain  ,.;()-_ and alpha-numeric characters.", "");
	    {
		if (breakonerror) {
		    return -1;
		}
	...
	if (checkthere(name, totalpartitionnames) != -1) {
	    controlerrorprint2("[PARTITIONS]", name, "", "A partitions class with this name has been specified twice.", "");
	    {
		if (breakonerror) {
		    return -1;
		}
	    ...
	    totalpartitionnames.push_back(name);
	}
    }

    if (mymy == 1) {
	controlerrorprint2("[PARTITIONS]", name, "", "This partitions block contains no commands.", "");
	{
	    if (breakonerror) {
		return -1;
	    }
	...
	int    ss2s = ss2.size() - 1;
	if (ss1[0] != '[') {
	    controlerrorprint2("[PARTITIONS]", name, "", "First statement in a partitions block must begin with a [. check input.", "");
	    {
		if (breakonerror) {
		    return -1;
		}
	...
	if (ss2[ss2s] != ']') {
	    controlerrorprint2("[PARTITIONS]", name, "", "Last statement in a partitions block must end with a ]. check input.", "");
	    {
		if (breakonerror) {
		    return -1;
		}
	...
	if (count == 1) {
	    ...
	    if (!allAinB(s, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890,.;()-_")) {
		controlerrorprint2("[SITES]", name, "", "The tree name should only contain  ,.;()-_ and alpha-numeric characters.", "");
		{
		    if (breakonerror) {
			return -1;
		    }
	    ...
	    if (treepos == -1) {
		controlerrorprint2("[PARTITIONS]", name, "", "No tree named \"" + treename + "\" has been defined.", "");
		{
		    if (breakonerror) {
			return -1;
		    }
		...
		isitrandom = (*testtree).treerandom;
		if (isitrandom) {
		    overallrandom = true;
		}
//				isitrandomvec.push_back(isitrandom);
	    ...
	    if (!allAinB(s, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890,.;()-_")) {
		controlerrorprint2("[SITES]", name, "", "The model/branch class/site class name should only contain  \n,.;()-_ and alpha-numeric characters.", s);
		{
		    if (breakonerror) {
			return -1;
		    }
		...
		if (mbspos == -1) {
			...
			if (isitrandom && (*s).therearebranches) {
			    ...
			    } else if (strippedtree.size() != strippedtree2.size())   {
				anerror = true;
			    } else   {
				for (int we = 0; we < strippedtree.size(); we++) {
				    char c1 = strippedtree[we];
				    char c2 = strippedtree2[we];
				    if (c1 != c2) {
					anerror = true;
					break;
				    }
			    ...
			    if (anerror) {
				controlerrorprint2("[PARTITIONS]", name, "", "Guide tree (1) does not match the tree used in \nsites class " + mbname + " (2).\n1) " + strippedtree + "\n2) " + strippedtree2 + "\nThe trees must have their taxa/branches written in identical order.", "");
				{
				    if (breakonerror) {
					return -1;
				    }
		    ...
		    bool anerror = false;
		    if (strippedtree.size() != strippedtree2.size()) {
			anerror = true;
		    } else   {
			for (int we = 0; we < strippedtree.size(); we++) {
			    char c1 = strippedtree[we];
		...
		if (rootlength == 0) {
		    controlerrorprint2("[PARTITIONS]", name, "", "The root length must be an integer greater than zero.", s);
		    {
			if (breakonerror) {
			    return -1;
			}
...
int dealwithevolve(vector<string>& block)
{
    ...
    if (size == 0) {
	controlerrorprint2("[EVOLVE]", "", "", "The block is empty.", "");
	{
	    if (breakonerror) {
		return -1;
	    }
...
//		stringstream tt1,tt2,tt3; tt1<<j-2; tt2<<j-1; tt3<<j; t1=tt1.str(); t2=tt2.str(); t3=tt3.str();
	...
	if (!allAinB(s1, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890,.;()-_")) {
	    controlerrorprint2("[EVOLVE]", "", "", "Partition class names should only contain  ,.;()-_ \nand alpha-numeric characters.", s1);
	    {
		if (breakonerror) {
		    return -1;
		}
	...
	if (ans == -1) {
	    controlerrorprint2("[EVOLVE]", "", "", "No partitions block called " + s1 + " has been defined.", "");
	    {
		if (breakonerror) {
		    return -1;
		}
	...
	if (!allAinB(s2, "123456780")) {
	    controlerrorprint2("[EVOLVE]", "", "", "Number of repetitions must be an integer value", s2);
	    {
		if (breakonerror) {
		    return -1;
		}
	...
	if (!allAinB(s3, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890,.;/()-_")) {
	    controlerrorprint2("[EVOLVE]", "", "", "Filename stubs should only contain  ,.;()-_ and alpha-numeric characters.", s3);
	    {
		if (breakonerror) {
		    return -1;
		}
		...
		if ((totalevolve.at(i)).filenamestub == filenamestub) {
		    error = true;
		}
	    }
	    if (error) {
		controlerrorprint2("[EVOLVE]", "", "", "Several lines in the [EVOLVE] block contain the same filename stub.\nThis will cause output files to be overwritten - please correct.", s3);
		{
		    if (breakonerror) {
			return -1;
		    }
...
int parse_control_file(const std::string& masterfilename)
{
	...
	char c = if1.get();
	temp1.push_back(c);
	originalcontrol.push_back(c);
    }                       // parse control file character by character into temp1
	...
	if ((temp1.at(th1) == '/') && (temp1.at(th1 + 1) == '*')) {
	    WR = false;
	    mymycount++;
	}                                                                          ///// *1

	if (WR) {
	    temp2.push_back(temp1.at(th1));
	}

	if ((temp1.at(th1 - 1) == '*') && (temp1.at(th1) == '/')) {
	    WR = true;
	    mymycount--;
	}                                                                          ///// *2

	if (mymycount < 0) {
	    controlerrorprint2("CONTROL FILE", "", "", "There is a '*/' statement without a preceding\nand matching '/*' statement! ", "");
	    return -1;
	}
    }
	...
	if ((sq1 == "[TYPE]") || (sq1 == "[SETTINGS]") || (sq1 == "[MODEL]") || (sq1 == "[SITES]") || (sq1 == "[BRANCHES]") || (sq1 == "[PARTITIONS]") || (sq1 == "[EVOLVE]") || (sq1 == "[BRANCHES*]")) {
	    weareonbaby = false;
	}
	if (sq1 == "[TREE]") {
	    weareonbaby = true;
	...
	for (int sd = 0; sd < sv.size() - 1; sd++) {
	    ...
	    if (weareonbaby && (sq2[0] == '(')) {
		if ((sq1 != "M3") && (sq1 != "M2") && (sq1 != "M1") && (sq1 != "M0") && (sq1 != "ECMrest") && (sq1 != "ECMunrest")) {
		    sv.insert(sv.begin() + sd + 1, "M3");
		    sd++;
		}
		weareonbaby = false;
	    }
	...
	if (oldtest == "[submodel]") {
	    ...
	    stringstream sss;
	    string       ss;
	    for (int pg = 0; pg < replacenames.size(); pg++) {
		if (replacenames.at(pg) == test) {
		    sss << pg;
		    ss        = sss.str();
		    sv.at(qw) = ss;
		    error     = false;
    ...
    if (sv.at(2) == "1") {
	oldmethod = false;                 //old model
    } else if (sv.at(2) == "2")   {
	oldmethod = true;                  //new model
    } else   {
	isthereanerror = -1;
	controlerrorprint2("[TYPE]", "", "", "Second value for [TYPE] command must 1 or 2 for choice of algorithm", "");
	return -1;
    }
	...
	for (int i = 3; i < sv.size(); i++) {
		...
		if (thisbit == MAIN[j]) {
		    if (mytype == 0) {
			settingsV = rough;
			numberofsettingsblocks++;
		    } else if (mytype == 1)   {
			modelsV.push_back(rough);
			numberofmodelsblocks++;
		    } else if (mytype == 2)   {
			sitesV.push_back(rough);
		    } else if (mytype == 3)   {
			branchesV.push_back(rough);
			branchesCV.push_back(true);
		    } else if (mytype == 4)   {
			treesV.push_back(rough);
		    } else if (mytype == 5)   {
			partitionsV.push_back(rough);
		    } else if (mytype == 6)   {
			branchesV.push_back(rough);
			branchesCV.push_back(false);
		    }
	...
	dealwithsettings(settingsV);
    }

    // runs through commands in correct order, and skips if an error occurs anywhere
    for (p = 0; p < modelsV.size(); p++) {
	if (isthereanerror == -1) {
	    return -1;
	} else   {
	    isthereanerror = dealwithmodel(modelsV.at(p));
	}
    }
    for (p = 0; p < branchesV.size(); p++) {
	if (isthereanerror == -1) {
	    return -1;
	} else   {
	    isthereanerror = dealwithbranches(branchesV.at(p), branchesCV.at(p));
	}
    }
    for (p = 0; p < sitesV.size(); p++) {
	if (isthereanerror == -1) {
	    return -1;
	} else   {
	    isthereanerror = dealwithsites(sitesV.at(p));
	}
    }
    for (p = 0; p < treesV.size(); p++) {
	if (isthereanerror == -1) {
	    return -1;
	} else   {
	    isthereanerror = dealwithtrees(treesV.at(p));
	}
    }
    for (p = 0; p < partitionsV.size(); p++) {
	if (isthereanerror == -1) {
	    return -1;
	} else   {
