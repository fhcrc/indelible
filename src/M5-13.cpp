 ...
 */
    ...
    case (NS2gamma):
	cdf = p[0] * CDFGamma(x, p[1], p[2]) + (1 - p[0]) * CDFGamma(x, p[3], p[3]);
	break;
    ...
    case (NSbetagamma):
	cdf = p[0] * CDFBeta(x, p[1], p[2], 0) + (1 - p[0]) * CDFGamma(x, p[3], p[4]);
	break;
...
//	cout<<"!!!!!!!!!! !"<<endl;
	...
	output.push_back(com.rK[i]);
    }

    for (int pp = 0; pp < ngamcat; pp++) {
	freqs.push_back(com.freqK[pp]);
    }
 ...
 */
	...
	vector<vector<string> > paramlist;
	vector<string>          blah;
	blah.push_back("alpha");
	blah.push_back("beta");
	paramlist.push_back(blah);
	blah.clear();
	blah.push_back("p0");
	blah.push_back("alpha1");
	blah.push_back("beta1");
	blah.push_back("alpha2 (=beta2)");
	paramlist.push_back(blah);
	blah.clear();
	blah.push_back("p_beta");
	blah.push_back("q_beta");
	paramlist.push_back(blah);
	blah.clear();
	blah.push_back("p0");
	blah.push_back("p_beta");
	...
	blah.push_back("p_beta");
	blah.push_back("q_beta");
	blah.push_back("w=1 fixed");
	paramlist.push_back(blah);
	blah.clear();
	blah.push_back("p0");
	blah.push_back("p_beta");
	blah.push_back("q_beta");
	blah.push_back("w>=1 estimated");
	paramlist.push_back(blah);
	blah.clear();
	...
	while (true) {
		...
		if ((modelnumber > 4) && (modelnumber < 14)) {
		    break;
		} else   {
		    cout << endl << "    That integer is outside the accepted range." << endl;
		}
	    } else   {
		cout << endl << "    That is not an integer! " << endl;
	    }
	}
	...
	blah = paramlist.at(modelnumber - 5);
	for (int yg2 = 0; yg2 < blah.size(); yg2++) {
	    cout << "    Parameter " << yg2 + 1 << ":\t" << blah.at(yg2) << endl;
	}
	cout << endl;
	...
	for (int pi = 0; pi < mysize; pi++) {
	    ...
	    if ((modelnumber == 11) && (pi == 3)) {
		check += "-";
	    }

	    if (((modelnumber == 12) || (modelnumber == 13)) && (pi == 2)) {
		check += "-";
	    }

	    if (!itis(readin, check)) {
		if (((modelnumber == 12) || (modelnumber == 13)) && (pi == 2)) {
		    cout << endl << "      ERROR: this parameter must be a real number. Allowed characters are:" << endl << "      -.0123456789" << endl;
		} else if ((modelnumber == 11) && (pi == 3))   {
		    cout << endl << "      ERROR: this parameter must be a real number. Allowed characters are:" << endl << "      -.0123456789" << endl;
		} else   {
		    cout << endl << "      ERROR: this parameter must be a decimal number. Allowed characters are:" << endl << "      .0123456789" << endl;
		}
		...
		temp = atof(readin.c_str());
	    }


	    // check value is not incorrect
	    if ((modelnumber == 5) || (modelnumber == 7)) {
		if (temp == 0) {
		    cout << endl << "      ERROR: this parameter must be greater than zero. Try again." << endl << endl;
		    pi--;
		    continue;
		}
	    } else   {
		if (((modelnumber == 12) || (modelnumber == 13)) && (pi == 2)) {
		...
		else if ((modelnumber == 8) && (pi == 3)) {
		}                                             // omega
		else {
		    if (pi == 0) {
			if (temp > 1) {
			    cout << endl << "      ERROR: proportion p0 must be between 0 and 1.  You entered " << temp << endl << "      Try again." << endl << endl;
			    pi--;
			    continue;
			}
		    } else if (pi == 1)   {
			if ((modelnumber == 12) || (modelnumber == 13)) {
			    if (temp > 1) {
				cout << endl << "      ERROR: proportion p1 must be between 0 and 1.  You entered " << temp << endl << "      Try again." << endl << endl;
				pi--;
				continue;
			    }

			    if (temp + params.at(0) > 1) {
				cout << endl << "      ERROR: sum of proportions p0 and p1 must be between 0 and 1.\n    You entered " << temp + params.at(0) << endl << "      Try again." << endl << endl;
				pi--;
				continue;
			    }
			} else if (temp == 0)   {
	...
	for (int iu = 0; iu < params.size(); iu++) {
	    mypar[iu] = params.at(iu);
	}

	DiscreteNSsites(mypar, ngamcat, modelnumber, output2, freqs2);

	vector<string> output, freqs;
	for (int tg = 0; tg < ngamcat; tg++) {
	    stringstream sd1;
	    sd1 << output2.at(tg);
	    output.push_back(sd1.str());
	    stringstream sd2;
	    sd2 << freqs2.at(tg);
	    freqs.push_back(sd2.str());
	}
	...
	blah = paramlist.at(modelnumber - 5);
	for (int yg1 = 0; yg1 < blah.size(); yg1++) {
	    of1 << "    Parameter " << yg1 + 1 << ":\t" << blah.at(yg1) << "\t" << params.at(yg1) << endl;
	}
	of1 << "*/" << endl << endl;
