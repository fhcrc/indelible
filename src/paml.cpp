 ...
 */
    ...
    for (i = 0; i < n; i++) {
	for (j = 0; j < n; j++) {
	    row.push_back(Q[(i * n) + j]);
	}
	for (j = 1; j < n; j++) {
	    row.at(j) += row.at(j - 1);
	}
	double diff = row.at(n - 1) - 1;
	if (diff < 0) {
	    diff = -diff;
	}
	if (diff > 0.00001) {
	    cout << "matexp row didn't sum to 1 " << diff << endl;
	    for (j = 0; j < n; j++) {
		cout << row.at(j) << "\t";
	    }
	    cout << endl;
 ...
 */
	...
	if (diff > 0.00001) {
	    cout << "Pmat row didn't sum to 1 " << diff << endl;
	    for (j = 0; j < n; j++) {
		cout << row.at(j) << "\t";
	    }
	    cout << endl;
...
string OutSubTreeN(string& subtree, FILE *fout, int inode, int spnames, int printopt)                                  //, char *labelfmt)
{
	...
	if ((printopt & PrNodeNum) && nodes[ison].nson) {
	    ...
	    tempstring = fd4.str();
	    subtree   += tempstring;
	}                                                                                                                   //fprintf(tempout," %d ",ison+1);   /* fprintf(fout,"%d",ison+1-sptree.nspecies);  */
	if ((printopt & PrLabel) && (nodes[ison].label > 0)) {
	    ///////////////HERE
	    stringstream fd5;
	    fd5 << nodes[ison].label;
	    tempstring = fd5.str();
	    subtree   += tempstring;
	}                                                                                                                         //subtree+=nodes[ison].label; //  {stringstream fd5; fd5<<nodes[ison].label; tempstring=fd5.str(); subtree+=tempstring;}  //fprintf(tempout, labelfmt, nodes[ison].label);
	if ((printopt & PrAge) && nodes[ison].age) {
	    stringstream fd6;
	    fd6 << nodes[ison].age;
	    tempstring = fd6.str();
	    subtree   += tempstring;
	}                                                                                                                            //fprintf(tempout, " @%.3f", nodes[ison].age);
	...
	if (printopt & PrBranch) {
	    stringstream fd7;
	    fd7 << nodes[ison].branch;
	    tempstring = fd7.str();
	    subtree   += ':';
	    subtree   += tempstring;
	}                                                                                                                                                                //fprintf(tempout,": %.6f", nodes[ison].branch);

	if (i < nodes[inode].nson - 1) {
	    subtree += ",";                                                       //fprintf(tempout,", ");
	}
    }
    subtree += ")";                              //fputc (')', fout);
...
string OutaTreeN(FILE *fout, int spnames, int printopt)
{
	...
	tempstring = fd8.str();
	subtree   += tempstring;
    }                                                                                                                                      //fprintf(tempout," %d ", tree.root+1);
    if ((printopt & PrLabel) && (nodes[tree.root].label > 0)) {
	stringstream fd5;
	fd5 << nodes[tree.root].label;
	tempstring = fd5.str();
	subtree   += tempstring;
    }                                                                                                                                 //subtree+=nodes[tree.root].label;//fprintf(tempout, labelfmt[intlabel], nodes[tree.root].label);
    if ((printopt & PrAge) && nodes[tree.root].age) {
	stringstream fd10;
	fd10 << nodes[tree.root].age;
	tempstring = fd10.str();
	subtree   += tempstring;
    }                                                                                                                                 //fprintf(tempout, " @%.3f", nodes[tree.root].age);

    if ((printopt & PrBranch) && (nodes[tree.root].branch > 1e-8)) {
	stringstream fd11;
	fd11 << nodes[tree.root].branch;
	tempstring = fd11.str();
	subtree   += ":";
	subtree   += tempstring;
    }                                            //fprintf(tempout,": %.6f", nodes[tree.root].branch);
...
string newrandomtree(int ntaxa, double birth, double death, double sample, double mut, int randomseed, int option)
{
    ...
    if (com.ns > NS) {
	error2("Too many species.  Raise NS.");
    }
	...
	if ((com.ns < 20) && (ntree < 10)) {
	    randomtree = OutaTreeN(F0, 0, BD);
	} else   {
	    randomtree = OutaTreeN(fout, 1, BD);
	}
		  ...
		  double alfa, double beta, int K, int median)
{
    ...
    if (median == 1) {
	for (i = 0; i < K; i++) {
	    rK.at(i) = PointGamma((i * 2.0 + 1) * gap05, alfa, beta);
	}
	for (i = 0, t = 0; i < K; i++) {
	    t += rK.at(i);
	}
	...
	for (i = 1; i < K - 1; i++) {
	    rK.at(i) = (freqK.at(i) - freqK.at(i - 1)) * factor / (1 - pinv);
	}
    }
    for (i = 0; i < K; i++) {
	freqK.at(i) = (1.0 - pinv) / K;
    }

    if (pinv > 0) {
	rK.push_back(0);
	freqK.push_back(pinv);
	cumfreqK.push_back(pinv);
    }


    cumfreqK.at(0) = freqK.at(0);

    for (i = 1; i < freqK.size(); i++) {
	cumfreqK.at(i) = freqK.at(i) + cumfreqK.at(i - 1);
    }

    double diff = cumfreqK.back() - 1;
    if (diff < 0) {
	diff = -diff;
    }
    if (diff > 0.00001) {
	cout << endl << endl << "diff is " << diff << "   ERROR IN DISCRETEGAMMA CUMFREQS" << endl << endl;
    }
//			cout<<"HERE"<<endl;
    return(0);
}
